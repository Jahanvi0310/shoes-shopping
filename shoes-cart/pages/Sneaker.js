...
export async function getServerSideProps(context) {
  ...
  for (let item of products)
    if (item.title in sneaker) {
      if (
        !sneaker[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        sneaker[item.title].color.push(item.color);
      }
      if (
        !sneaker[item.title].size.includes(item.size) &&
        item.availableQty > 0
      ) {
        sneaker[item.title].size.push(item.size);
      }
    } else {
      sneaker[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        sneaker[item.title].color = [item.color];
        sneaker[item.title].size = [item.size];
      }
    }

  return{
    props:{products:JSON.parse(JSON.stringify(sneaker))}
  }
}
